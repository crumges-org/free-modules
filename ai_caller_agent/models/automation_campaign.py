...
class automationCampaign(models.Model):
    ...
    delay_seconds = fields.Integer(
        ...
        default=0,
        help="Wait this many seconds before sending the next record to automation.",
    )
    ...
    # ---------------------------------------------------
    def _send_pending_leads_via_automation(self):
        """
        Send ALL matching leads to automation (always resend),
        ...
        if requests is None:
            raise UserError(
                _(
                    ...
                    "Please install it to send data to automation."
                )
            )

        for campaign in self:
            if not campaign.webhook_url:
                raise UserError(_("Please set the automation Webhook URL first."))
            ...
            _logger.info(
                "Sending %s records to automation webhook %s",
                len(leads),
                campaign.webhook_url,
            )

            Log = self.env["automation.campaign.log"]
            ...
            for lead in leads:
                ...
                try:
                    ...
                    _logger.exception("Error sending data to automation")
                    log.status = "error"
                    log.sent_at = fields.Datetime.now()
    ...
    def _run_campaign_in_thread(self):
        ...
        try:
            ...
            with self.env.registry.cursor() as new_cr:
                ...
                campaign = new_env['automation.campaign'].browse(self.id)
                ...
                campaign._send_pending_leads_via_automation()
    ...
    # ---------------------------------------------------
    def action_send_to_automation(self):
        """Manual trigger – send all matching leads now."""
        return self._send_pending_leads_via_automation()

    # ---------------------------------------------------
    # CRON ENTRY POINT
    # ---------------------------------------------------
    @api.model
    def _cron_run_automation_campaigns(self):
