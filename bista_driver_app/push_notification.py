...
def firebase_send_notification(self, payload_data):
    try:
        ...
        if base_url and re.search("localhost", base_url):
            return False
        
        # NOTE: For Fleet app separate firebase configuration is being used. following if/else condition is added to
        # decide which configuration to use. If bista_driver_app module is uninstalled following if block will
        # no longer be required.
         
        if self.env.context.get('is_use_fleet_firebase_configuration'):
            firebase_project_id_config_parameter_key = 'bista_driver_app.fleet_firebase_project_id'
            firebase_admin_key_file_config_parameter_key = 'bista_driver_app.fleet_firebase_admin_key_file'


        firebase_project_id = IrConfigParameter.get_param(firebase_project_id_config_parameter_key)
        firebase_admin_key_file = IrConfigParameter.get_param(firebase_admin_key_file_config_parameter_key)
        ...
        if service_account:
            ...
            for notification_data in payload_data:
                ...
                # T2181:server name with fcm topic
                server_env = IrConfigParameter.get_param('bista_driver_app.server_env')
                _logger.info(f'Firebase notification data :{notification_data}')
