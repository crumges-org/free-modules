...
_EH_SEALED_LINE_STORED_ALLOWLIST = frozenset({'blocked'})
...
class AccountMoveSend(models.TransientModel):
    ...
    def _process_send_and_print(
        self, moves, wizard=None, allow_fallback_pdf=False, **kwargs
    ):
        moves.sudo(False)._eh_check_access('write')
        ...
        return super(AccountMoveSend, guarded)._process_send_and_print(
            guarded_moves,
            # The transient wizard computes JSON warning/action payloads.
            # Never contaminate that environment with an object capability:
            # the guarded worker and moves are sufficient for every
            # server-owned account.move/attachment write below.
            wizard=wizard,
    ...
    def action_send_and_print(
        ...
        self.ensure_one()
        actor_moves = self.move_ids.sudo(False)
        actor_moves._eh_check_access('write')
        if actor_moves.filtered('eh_legacy_unverified_seal'):
            raise UserError(_(
                "Unverified legacy journal entries cannot enter the invoice "
                "sending workflow."
            ))

        # Odoo 17 queues a multi-invoice send by writing its wizard values
        # directly onto account.move before triggering the cron.  Keep the
        # wizard JSON context capability-free and route only that exact
        # server-owned queue field through Base's private metadata corridor.
        process_later = (
            self.mode == 'invoice_multi'
            and not (force_synchronous or self.checkbox_download)
        )
        if process_later:
            if self.checkbox_send_mail and not self.mail_template_id:
                raise UserError(_(
                    "Please select a mail template to send multiple invoices."
                ))
            actor_moves._eh_write_sealed_metadata({
        ...
        # Core calls our _process_send_and_print override synchronously. Keep
        # this wizard recordset clean; that worker narrows capabilities to
        # itself and the account.move recordset only.
        return super(AccountMoveSend, self).action_send_and_print(
...
class AccountMoveSendCron(models.Model):
    _inherit = 'account.move'
    ...
    def _cron_account_move_send(self, job_count=10):
        ...
        return super(AccountMoveSendCron, guarded)._cron_account_move_send(
            job_count=job_count,
        )
