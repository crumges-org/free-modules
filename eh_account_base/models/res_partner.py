...
class ResPartner(models.Model):
    ...
    @api.depends_context(
        'uid', 'allowed_company_ids', 'company',
    )
    def _compute_application_statistics(self):
        statistics_by_partner = (
            self._compute_application_statistics_hook() or {}
        )
        sanitizer = self.env[
            'eh.list.statistics.mixin'
        ]._eh_sanitize_list_statistics
        for partner in self:
            partner.application_statistics = sanitizer(
                statistics_by_partner.get(partner.id, []),
            )

    def _compute_application_statistics_hook(self):
        return {partner.id: [] for partner in self}
