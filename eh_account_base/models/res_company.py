...
class EhAccountReportCompanyVersion(models.Model):
    ...
    _sql_constraints = [
        ('company_unique', 'unique(company_id)', 'Each company has one accounting report input version.'),
    ]
...
class ResCompany(models.Model):
    ...
    _sql_constraints = [
        ('positive_report_row_limit', 'CHECK (eh_gl_row_limit > 0)', 'GL Row Limit must be greater than zero.'),
        ('positive_expand_page_size', 'CHECK (eh_expand_page_size > 0)', 'Lazy Expand Page Size must be greater than zero.'),
        ('expand_page_within_row_limit', 'CHECK (eh_expand_page_size <= eh_gl_row_limit)', 'Lazy Expand Page Size cannot exceed GL Row Limit.'),
    ]
