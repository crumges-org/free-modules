...
patch(Thread.prototype, {
        ...
        try {
            const msgs = await this.orm.searchRead(
                ...
                [
                    ...
                    "subtype_id", "attachment_ids", "starred_partner_ids", "subject",
                ],
            ...
            // Resolve subtype "internal" flags by id — matching translated labels breaks i18n
            let subtypeInternalMap = {};
            try {
                const subtypeIds = [...new Set(msgs.map(m => m.subtype_id?.[0]).filter(Boolean))];
                if (subtypeIds.length) {
                    const subtypeRecs = await this.orm.searchRead(
                        "mail.message.subtype",
                        [["id", "in", subtypeIds]],
                        ["internal"]
                    );
                    subtypeRecs.forEach(s => { subtypeInternalMap[s.id] = !!s.internal; });
                }
            } catch (e) {
                console.error("[Chatter] subtype load error", e);
            }
            ...
            const normalised = msgs.map(m => {
                ...
                const subtypeInternal = subtypeInternalMap[m.subtype_id?.[0]];
                const isNote = !isLog && (subtypeInternal !== undefined
                    ? subtypeInternal
                    : (subtypeName.includes("note") || subtypeName.includes("internal")));
    ...
    async scToggleStar(id) {
        ...
        try {
            ...
            this.notification.add(_t("Could not star message"), { type: "danger" });
        }
    },
    ...
    async scSaveEdit(id) {
        ...
        try {
            ...
            this.notification.add(_t("Could not save"), { type: "danger" });
        }
    },
    ...
    async scDeleteMsg(id) {
        ...
        try {
            ...
            this.notification.add(_t("Deleted"), { type: "success" });
        } catch (_) {
            this.notification.add(_t("Could not delete"), { type: "danger" });
        }
    },
    ...
    async scDeleteAttachment(msgId, attId) {
        ...
        try {
            ...
            this.notification.add(_t("Could not remove"), { type: "danger" });
        }
    },
    ...
    async scToggleReaction(msgId, emoji) {
        ...
        try {
            ...
            this.notification.add(_t("Could not update reaction"), { type: "danger" });
        }
    },
