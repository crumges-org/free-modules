...
class Traktop(models.Model):
    ...
    #     # ─── Build our UTC "now" and "3 days from now" strings ────────────────────
    #     now_dt = datetime.utcnow()
    #     end_dt = now_dt + timedelta(days=3)
    #     now_str = fields.Datetime.to_string(now_dt)
    #     end_str = fields.Datetime.to_string(end_dt)

    #     # ─── Fetch only outgoing pickings whose Sale Order delivery date is in the next 3 days ───
    #     deliveries = self.env['stock.picking'].sudo().search([
    #         ('state', '=', 'assigned'),
    #         ('picking_type_id.code', '=', 'outgoing'),
    #         ('sale_id.commitment_date', '>=', now_str),
    #         ('sale_id.commitment_date', '<=', end_str),
    #     ])
    #     _logger.info("fetch_jobs_data: Found %s delivery orders in 'assigned' state within 3 days", len(deliveries))

    #     job_data = []
    #     for picking in deliveries:
    #         partner = picking.partner_id
    #         if not (partner and partner.partner_latitude and partner.partner_longitude):
    #             continue

    #         # Base coords
    #         lon, lat = partner.partner_longitude, partner.partner_latitude

    #         # Build the VROOM time window (08:00–18:00) on the commitment_date
    #         sched = fields.Datetime.from_string(picking.sale_id.commitment_date)
    #         start_ts = int(datetime.combine(sched.date(), datetime.min.time()).replace(hour=8).timestamp())
    #         end_ts   = int(datetime.combine(sched.date(), datetime.min.time()).replace(hour=18).timestamp())
    ...
    #     # If no jobs, return empty list (you’ll want to guard against calling VROOM with zero jobs)
    #     return job_data
    @api.model
    def fetch_jobs_data(self):
        deliveries = self.env['stock.picking'].sudo().search([
 ...
 ########################################################
    ...
    #         # Unlink only auto-generated records (those that have not been manually overridden).
    #         auto_records = self.search([('manual_vehicle_override', '=', False)])
    #         auto_records.unlink()

    #         # ----------------------------------------------------------
    #         # Merge/Group all steps by vehicle.
    #         # ----------------------------------------------------------
    #         from collections import defaultdict
    ...
    #             # Merge steps for a given vehicle.
    #             steps_by_vehicle[vehicle_id].extend(steps)

    #         # ----------------------------------------------------------
    #         # Create (or update) optimized records per vehicle using a single route_id.
    #         # ----------------------------------------------------------
    #         # We'll use an incremental route index (alternatively, you might use the vehicle_id).
    #         for route_idx, (vehicle_id, steps) in enumerate(steps_by_vehicle.items()):
    #             vehicle_rec = self.env['fleet.vehicle'].search([("id", "=", vehicle_id)], limit=1)
    #             if not vehicle_rec:
    ...
    #                     # Use the same route index for all steps for this vehicle.
    #                     'route_id': route_idx,
    ...
    #                             'build_time': delivery_order.sale_id.build_time or 0,
    #                         })
    #                     # Check for an existing record to avoid duplicates.
    #                     existing_record = self.search([('delivery_order_id', '=', job_id)], limit=1)
    #                     if existing_record:
    #                         if existing_record.manual_vehicle_override:
    #                             # Instead of skipping creation, update the existing manual record
    #                             # with the new route assignment (route_id and route_sequence).
    #                             existing_record.with_context(from_optimization=True).write({
    #                                 'route_id': route_idx,
    #                                 'route_sequence': step_idx,
    #                             })
    #                             continue
    ...
    #                 # Create the new record with a context flag so that the write() method
    #                 # does not mark it as manual override.
    #                 self.with_context(from_optimization=True).create(vals)

    #         # ----------------------------------------------------------
    #         # Finally, update the route sequence (e.g., via VROOM API per vehicle).
    #         # ----------------------------------------------------------
    ...
    #             'params': {
    #                 'title': _('Success'),
    #                 'message': _('Optimized routes have been successfully created/updated. Manual overrides were maintained.'),
    #                 'type': 'success',
    #                 'sticky': False,
    #                 'next': {
    #                     'type': 'ir.actions.client',
